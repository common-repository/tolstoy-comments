...
class Plugin
{
	...
	public static $pluginFolder = 'tolstoy-comments';
	...
	{
		load_plugin_textdomain(static::$pluginSlug, false, static::$pluginFolder.'/languages/');
	}
	...
	{
		add_menu_page(static::$pluginName, sprintf('%s <span class="awaiting-mod" style="visibility:hidden" data-tolstoycomments>0</span>', static::$pluginShortName), 'moderate_comments', static::$pluginSlug, function() {}, 'dashicons-admin-comments', 76);
	...
	public static function commentsTemplate($template)
	{
		if(get_option('tolstoycomments_active') == 1) {
			if(get_option('tolstoycomments_index') == 1) {
				global $wp_query, $withcomments, $post, $wpdb, $id, $comment, $user_login, $user_ID, $user_identity;
				$comments_by_type = $wp_query->comments_by_type;

				if (file_exists($template)){
	
					echo '<div style="display:none;" data-source-theme="tolstoycomments">';
					require $template;
					echo '</div><!-- .comments-area-theme-tolstoycomments -->';
				}
				else
				{
					echo '<div id="comments" style="display:none;" class="comments-area" data-source-template="tolstoycomments">';
					...
					echo '</div><!-- .comments-area-template-tolstoycomments -->';
				}
			}
