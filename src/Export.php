...
class Export
{
	...
	public function handle($lastId)
	{
		...
		// Admin::httpGetContents();
		// $result = file_get_contents($url);
		$result = Admin::httpGetContents($url);
		...
		foreach($result->data->comments as $i) {
			$postId = url_to_postid($i->chat->url);
			...
			elseif($wpdb->get_var($wpdb->prepare("select 1 from $wpdb->posts where `ID` = %d limit 1", $postId)))
			{
				// write_log('insert');
